(function () {
	...
	container.className = "kiki-bouba-toggle";
	container.innerHTML = `
		<button class="toggle-btn" id="toggleBtn" aria-label="Toggle kiki/bouba theme">
			<svg class="shape" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
	...
	style.textContent = `
		...
		.toggle-btn:hover .morph-path { fill: #111; }
		@media (max-width: 560px) {
			.kiki-bouba-toggle { top: 12px; right: 12px; }
			.toggle-btn { width: 48px; height: 48px; }
		}
	`;
	...
	const kikiToBouba = [kiki, middle1, middle2, middle3, bouba];
	const boubaToKiki = [bouba, middle3, middle2, middle1, kiki];
	...
	function animate(frames, done) {
		...
		(function step() {
			if (i < frames.length) {
				morphPath.setAttribute("d", frames[i++]);
	...
	let isKiki = localStorage.getItem("theme") === "kiki";
	...
	toggleBtn.addEventListener("click", () => {
		...
		const frames = isKiki ? kikiToBouba : boubaToKiki;
		animate(frames, () => { toggleBtn.disabled = false; });
		isKiki = !isKiki;
		document.body.classList.toggle("theme-kiki", isKiki);
		localStorage.setItem("theme", isKiki ? "kiki" : "bouba");
	});
})();
